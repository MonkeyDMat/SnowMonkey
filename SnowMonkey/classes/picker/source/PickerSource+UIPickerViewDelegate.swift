...
extension PickerSource: UIPickerViewDelegate {
    ...
    public func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        guard let picker = picker,
              ...
              row < components[component].rows.count else {
            return
        }
        
        let componentObj = components[component]
        let row = componentObj.rows[row]
        
        guard let value = row.text ?? row.attributedText?.string else {
            return
        }
        
        delegate?.didSelectRow(picker: picker, rowId: row.id, value: value, inComponent: component)
    }
}
